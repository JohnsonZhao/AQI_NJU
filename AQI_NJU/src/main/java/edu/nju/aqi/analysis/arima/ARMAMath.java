...
public class ARMAMath {
	...
	public double[] autocorData(double[] dataArray, int order) {
		...
		double varData = this.varerrData(dataArray);// 标准化后的方差
	...
	public double[] parcorrCompute(double[] dataArray, int p, int q) {
		...
		// System.out.println("row="+parautocorDataMatrix.getRowDimension()+"
		// Col="+parautocorDataMatrix.getColumnDimension());
		...
		result[result.length - 1] = atuocorr[0] - sum2; // result数组最后一个存储干扰估计值ֵ

		return result; // 返回0列的最后一个就是k阶的偏自相关系数 pcorr[k]=返回值ֵ
	}
}
